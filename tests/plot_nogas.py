...
changa_snaps = ['ChaNGa/nogas/nogas.tipsy', 'ChaNGa/nogas/nogas.out.000050.tipsy', 'ChaNGa/nogas/nogas.out.000300.tipsy']
gadget_snaps = ['Gadget3/nogas/nogas.hdf5', 'Gadget3/nogas/nogas_000.hdf5', 'Gadget3/nogas/nogas_005.hdf5']
...
for i in range(0, len(snapshots)):
    ...
    plotting.mesh(grid[i], disk.x_grid, disk.y_grid, disk.stellar_smd, vrange, cmap=cmap, cbar_ax=grid.cbar_axes[i],
                  ...
                  title=titles(i))
    if i%2==0:
        grid[i].text(-5.0, 15.0, r'$t=' + '{0:.2f}'.format(disk.time) + r'\,\rm{Gyrs}$', fontsize=6)
